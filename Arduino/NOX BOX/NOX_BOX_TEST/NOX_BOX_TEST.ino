...
const int t = 5;
long chrono1 = millis();
long attente = 5000; // attente avant d'éteindre en milliseconde
...
void setup() {
  ...
  pinMode(mouv2, INPUT);
  pinMode(red, OUTPUT);
  servo2.attach(10); // allumage
  servo1.attach(11); // éteignage
  Serial.begin(9600);
  servo1.write(118);
  servo2.write(118);
}
...
void loop() {
  bool state_mouv = (digitalRead(4)) || (digitalRead(5)); // booléen détection mouv 
  
  bool capteurOrange = digitalRead(4);
  bool capteurBleu = digitalRead(5);
  Serial.println(capteurBleu);
  
  bool state_led = digitalRead(7);
